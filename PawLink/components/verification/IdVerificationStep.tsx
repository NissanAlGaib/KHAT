...
export default function IdVerificationStep({
  ...
  const [idType, setIdType] = useState<string>(
    (initialData.idType as string) || "",
  );
  const [idPhoto, setIdPhoto] = useState<string | null>(
    (initialData.idPhoto as string) || null,
  );
  const [name, setName] = useState<string>(
    (initialData.idName as string) || "",
  );
  const [idNumber, setIdNumber] = useState<string>(
    (initialData.idNumber as string) || "",
  );
  const [birthdate, setBirthdate] = useState<Date>(
    initialData.idBirthdate
      ? new Date(initialData.idBirthdate as string)
      : new Date(),
  );
  const [givenDate, setGivenDate] = useState<Date>(
    initialData.idGivenDate
      ? new Date(initialData.idGivenDate as string)
      : new Date(),
  );
