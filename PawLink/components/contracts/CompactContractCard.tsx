...
function getNextAction(
  contract: BreedingContract,
): { text: string; emoji: string; tab?: string } | null {
  const status = contract.status;

  if (status === "pending_review") {
    if (contract.can_accept) {
      return {
        text: "Review & accept the contract",
        emoji: "👀",
        tab: "overview",
      };
    }
  ...
  if (status === "accepted") {
    ...
    if (breedingStatus === "pending" || breedingStatus === "in_progress") {
      if (contract.can_mark_breeding_complete) {
        return {
          text: "Mark breeding as complete",
          emoji: "💕",
          tab: "breeding",
        };
      }
    ...
    if (breedingStatus === "completed" && contract.has_offspring) {
      if (contract.can_input_offspring) {
        return {
          text: "Record or allocate offspring",
          emoji: "🐾",
          tab: "offspring",
        };
      }
    ...
    if (breedingStatus === "failed") {
      return {
        text: "Breeding failed — review details",
        emoji: "😔",
        tab: "breeding",
      };
    }
