...
const statusConfig: Record<string, { emoji: string; label: string; color: string; bg: string }> = {
  pending: { emoji: "⏳", label: "Pending", color: "#eab308", bg: "#fefce8" },
  in_progress: { emoji: "💕", label: "In Progress", color: "#8b5cf6", bg: "#f5f3ff" },
  completed: { emoji: "✅", label: "Completed", color: "#16a34a", bg: "#f0fdf4" },
  failed: { emoji: "😔", label: "Failed", color: "#ef4444", bg: "#fef2f2" },
};

export default function ContractBreedingTab({ contract, onContractUpdated }: BreedingTabProps) {
  ...
  const handleSubmit = async () => {
    if (!breedingNotes.trim()) {
      showAlert({ title: "Required", message: "Please provide notes about the breeding outcome.", type: "error" });
      return;
    }
    ...
    showAlert({
      ...
      buttons: [
        ...
        try {
          ...
          if (result.success) {
            showAlert({
              title: formType === "completed" ? "Breeding Complete! 🎉" : "Breeding Marked as Failed",
              message: result.message || (formType === "completed" ? "The breeding has been marked as complete." : "The breeding has been recorded as failed."),
            ...
            showAlert({ title: "Error", message: result.message || "Failed to update breeding status.", type: "error" });
          }
        } catch (error) {
  ...
  if (contract.status !== "accepted" && contract.status !== "fulfilled") {
    return (
      <View className="items-center justify-center py-16 px-6">
        ...
        <Text className="text-gray-800 font-bold text-lg mb-2 text-center">Breeding Not Started</Text>
  ...
  return (
    <View className="px-4 pt-3">
      ...
      <View className="rounded-2xl p-5 mb-5 border" style={{ backgroundColor: config.bg, borderColor: `${config.color}30` }}>
      ...
      {contract.breeding_notes && (
        <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-100">
          <Text className="font-bold text-gray-800 text-sm mb-1">📝 Breeding Notes</Text>
          <Text className="text-gray-600 text-sm">{contract.breeding_notes}</Text>
        </View>
      )}
      ...
      {breedingStatus === "completed" && (
        <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-100">
          <View className="flex-row items-center">
            ...
            <Text className="font-bold text-gray-800 text-sm ml-2">Offspring Status</Text>
          </View>
          <View className="flex-row items-center mt-2">
            {contract.has_offspring ? (
              <>
                <CheckCircle2 size={16} color="#16a34a" />
                <Text className="text-green-700 text-sm ml-2">Offspring expected — go to the Offspring tab to record them</Text>
              </>
            ) : (
              <>
                <XCircle size={16} color="#eab308" />
                <Text className="text-yellow-700 text-sm ml-2">No offspring from this breeding</Text>
              </>
            )}
          </View>
        </View>
      ...
      {contract.can_mark_breeding_complete && breedingStatus !== "completed" && breedingStatus !== "failed" && (
        <View className="bg-blue-50 rounded-2xl p-4 mb-4 border border-blue-100">
          <Text className="text-blue-800 text-sm">
            ℹ️ As the {contract.is_shooter ? "shooter" : "sire pet owner"}, you can mark the
            breeding as complete or failed once the process is done.
          </Text>
        </View>
      )}

      {/* Action Buttons */}
      {contract.can_mark_breeding_complete && breedingStatus !== "completed" && breedingStatus !== "failed" && !showForm && (
        <View className="mb-4">
          <TouchableOpacity
            onPress={() => { setFormType("completed"); setShowForm(true); setHasOffspring(true); }}
            className="bg-[#FF6B6B] py-4 rounded-2xl flex-row items-center justify-center mb-3"
          >
            <CheckCircle2 size={20} color="white" />
            <Text className="text-white font-bold text-base ml-2">Mark Breeding as Complete 🎉</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => { setFormType("failed"); setShowForm(true); setHasOffspring(false); }}
            className="bg-white py-4 rounded-2xl flex-row items-center justify-center border-2 border-red-300"
          >
            <XCircle size={20} color="#ef4444" />
            <Text className="text-red-500 font-bold text-base ml-2">Mark as Failed 😔</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {showForm && (
        <View className="bg-white rounded-2xl p-5 border border-gray-100 mb-4">
          ...
          {formType === "completed" && (
            <View className="mb-4">
              <Text className="font-semibold text-gray-700 text-sm mb-2">Were there any offspring? 🐾</Text>
              <View className="flex-row">
                <TouchableOpacity
                  onPress={() => setHasOffspring(true)}
                  className={`flex-1 py-3 rounded-l-2xl border-2 ${
                    hasOffspring ? "bg-green-500 border-green-500" : "bg-white border-gray-200"
                  }`}
                >
                  <Text className={`text-center font-semibold ${hasOffspring ? "text-white" : "text-gray-600"}`}>
                    🐾 Yes
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  className={`flex-1 py-3 rounded-r-2xl border-2 border-l-0 ${
                    !hasOffspring ? "bg-gray-500 border-gray-500" : "bg-white border-gray-200"
                  }`}
                >
                  <Text className={`text-center font-semibold ${!hasOffspring ? "text-white" : "text-gray-600"}`}>
                    ❌ No
          ...
          <View className="mb-4">
            <Text className="font-semibold text-gray-700 text-sm mb-1">Notes *</Text>
            <TextInput
          ...
          <View className="flex-row">
            <TouchableOpacity
              onPress={() => { setShowForm(false); setBreedingNotes(""); }}
        ...
        <View className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
          <View className="flex-row items-center">
            ...
            <Text className="text-gray-500 text-sm ml-2">
              Waiting for the {contract.shooter_user_id ? "shooter" : "sire pet owner"} to mark breeding status.
            </Text>
          </View>
        </View>
      )}
